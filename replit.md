...
- **Core Features**:
    ...
    - **Multi-Month Billing Cycles**: Plans support configurable cycle lengths via `cycleMonths` field (1, 3, 6, 12, or custom). Duration = cycleMonths * 30 days. Classes and price are for the entire cycle, not per month. `paidAt`, `expiresAt`, `classesRemaining` dynamically managed. `planStatus` is computed at read time. Renewal resets the cycle.
