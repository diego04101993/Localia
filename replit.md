...
- **Core Features**:
    ...
    - **Branch Admin Dashboard**: Comprehensive dashboard with tabs for:
        ...
        - **Perfil Público**: Dedicated tab for managing public-facing branch profile: basic info (description, category), gallery (up to 5 images), location (up to 2 locations with name, address, Google Maps URL stored in `locations` JSONB on branches table; backward-compat with single `address`/`googleMapsUrl` fields), operating hours (Mon-Sun weekly schedule), services/products (with type: service|product, durationMinutes), and reviews summary (read-only).
