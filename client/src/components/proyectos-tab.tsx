              ...
              <Tabs defaultValue="summary" className="space-y-4">
                ...
                <TabsContent value="summary" className="space-y-4">
                  <div className="grid gap-4 lg:grid-cols-2">
                    ...
                    <Card className="border-border/70"><CardHeader className="pb-3"><CardTitle className="text-base">Costos del proyecto</CardTitle><CardDescription>Las compras a proveedor no se restan otra vez: el costo de mercancía proviene del snapshot de cada venta.</CardDescription></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Costo de mercancía</p><p className="font-semibold">{formatCurrencyMx(detailProject.summary.cogsTotal)}</p></div><div><Tooltip><TooltipTrigger asChild><button type="button" className="cursor-help text-left text-xs text-muted-foreground underline decoration-dotted">Envíos y logística</button></TooltipTrigger><TooltipContent>Gastos clasificados por su categoría como envío, logística o flete. Ya forman parte de los gastos del proyecto.</TooltipContent></Tooltip><p className="font-semibold">{formatCurrencyMx(detailProject.summary.shippingExpenses)}</p></div><div><p className="text-xs text-muted-foreground">Comisiones devengadas</p><p className="font-semibold">{formatCurrencyMx(detailProject.summary.accruedCommissions)}</p></div><div><p className="text-xs text-muted-foreground">Otros gastos</p><p className="font-semibold">{formatCurrencyMx(detailProject.summary.otherOperatingExpenses)}</p></div><div className="border-t pt-2 sm:col-span-2"><p className="text-xs text-muted-foreground">Total operativo estimado</p><p className="font-semibold">{formatCurrencyMx(detailProject.summary.cogsTotal + detailProject.summary.shippingExpenses + detailProject.summary.accruedCommissions + detailProject.summary.otherOperatingExpenses)}</p></div></CardContent></Card>
                    ...
                    <Card className="border-border/70"><CardHeader className="pb-3"><CardTitle className="text-base">Pendientes de pago</CardTitle><CardDescription>El total combina compras, gastos documentales y comisiones pendientes, sin mezclar ni duplicar sus saldos.</CardDescription></CardHeader><CardContent className="space-y-3"><div className="flex justify-between gap-3"><span className="text-sm text-muted-foreground">Compras pendientes</span><strong>{formatCurrencyMx(detailProject.summary.accountsPayablePurchases)}</strong></div><div className="flex justify-between gap-3"><span className="text-sm text-muted-foreground">Gastos documentales pendientes</span><strong>{formatCurrencyMx(detailProject.summary.accountsPayableOtherExpenses)}</strong></div><div className="grid gap-2 rounded-xl bg-muted/40 p-3"><div className="flex justify-between gap-3"><span className="text-sm text-muted-foreground">Comisiones devengadas</span><strong>{formatCurrencyMx(detailProject.summary.accruedCommissions)}</strong></div><div className="flex justify-between gap-3"><span className="text-sm text-muted-foreground">Comisiones pagadas</span><strong>{formatCurrencyMx(detailProject.summary.paidCommissions)}</strong></div><div className="flex justify-between gap-3"><span className="text-sm text-muted-foreground">Comisiones pendientes</span><strong>{formatCurrencyMx(detailProject.summary.pendingCommissions)}</strong></div></div><div className="flex justify-between gap-3 border-t pt-3"><span className="font-medium">Total pendiente de pagar</span><strong>{formatCurrencyMx(detailProject.summary.totalPendingPayable)}</strong></div><p className="text-xs text-muted-foreground">Compras + gastos documentales (incluida logística) + comisiones pendientes.</p></CardContent></Card>
                ...
                <TabsContent value="administration" className="space-y-4">
                  <Card className="border-border/70">
                    ...
                    <CardContent className="grid gap-3 sm:grid-cols-3">
                      <div><p className="text-xs text-muted-foreground">Base de gastos documentales</p><p className="font-semibold">{formatCurrencyMx(detailProject.summary.obligationBeforeTax)}</p></div>
